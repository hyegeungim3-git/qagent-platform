...
FROM equipment_master e
  ...
  LEFT JOIN tag_dictionary t          -- 포스프레임 태그 표준 사전(4,820개)
      ...
      planBySource: {
        building: {
          ...
          steps: [
            ...
            { op: 'Hash Left Join', detail: 'tag_dictionary (raw_tag) — 표준 태그 사전 4,820개 해시 적재',  rows: 38, ms: 14.8 },
   ...
   주소 표준화 후 협력사 마스터 일괄 등록 예정`,
      ...
      masterMapping: {
        ...
        apply: {
          ...
          note: '자동 확정 1,312개는 별칭 사전·단위 규칙으로 판정된 항목입니다. 나머지 522개는 계측기 사양 확인이나 담당 부서 회신이 필요해 사람 검토로 넘깁니다 — 유사도 기반 추정이라 실제 적용 시 ±15% 편차가 예상됩니다.',
        },
      },
    },
    /* ── 기술문서 요약 에이전트 ── */
...
5. 후속 조치: 치료 종료 후 30일 이내에 결과 보고서를 제출하며, 해당 작업의 위험성평가를 재실시하고 재발 방지 대책을 수립하여 안전환경팀에 공유합니다.`,
  ...
  /* ── SECURE 탭 제안 4종 ── */
  /* ── 작업지시 추적 시드 ──
     시나리오가 완주하면 문서번호로 자동 등록되지만, 데모 첫 진입에도
     '진행 중인 지시'가 보이도록 기존 세계관의 지시를 미리 깔아둔다.
     status는 발행 → 작업중 → 완료 → 검증완료 순으로만 진행된다. */
  workOrderSeed: [
    { docNo: "HBP-생산기술-2026-110", title: "윤활 자동 도포 장치 시범 적용 — 표면 결함률 관찰",
      source: "공정회의 결정사항", owner: "생산기술팀 정재호", due: "2026-04-03",
      status: "작업중", updatedAt: "2026-03-30 09:40",
      history: [
        { status: "발행", at: "2026-03-14 15:20", by: "생산기술팀 정재호" },
        { status: "작업중", at: "2026-03-30 09:40", by: "생산기술팀 정재호" },
      ] },
    { docNo: "HBP-품질-2026-081", title: "SUS304 코일 로트 전수검사 전환",
      source: "수입검사 부적합", owner: "품질관리부 한지민", due: "2026-03-25",
      status: "완료", updatedAt: "2026-03-25 16:10",
      history: [
        { status: "발행", at: "2026-03-23 11:05", by: "품질관리부 한지민" },
        { status: "작업중", at: "2026-03-24 14:20", by: "품질관리부 한지민" },
        { status: "완료", at: "2026-03-25 16:10", by: "품질관리부 한지민" },
      ] },
    { docNo: "HBP-안전-2026-034", title: "금형 교체(SMED) 작업 위험성평가 재실시",
      source: "안전 정기 점검", owner: "안전환경팀 오세훈", due: "2026-04-10",
      status: "발행", updatedAt: "2026-03-31 17:30",
      history: [{ status: "발행", at: "2026-03-31 17:30", by: "안전환경팀 오세훈" }] },
    { docNo: "HBP-보전-2026-095", title: "침탄로 3호기 후단존 열전대 재교정",
      source: "설비 이상 진단", owner: "설비보전팀 김도현", due: "2026-03-18",
      status: "검증완료", updatedAt: "2026-03-19 10:00",
      history: [
        { status: "발행", at: "2026-03-17 08:15", by: "AI 자동 발행" },
        { status: "작업중", at: "2026-03-17 13:00", by: "설비보전팀 김도현" },
        { status: "완료", at: "2026-03-18 15:40", by: "설비보전팀 김도현" },
        { status: "검증완료", at: "2026-03-19 10:00", by: "품질관리부 한지민" },
      ] },
  ],
  workOrderNote: "자동화 시나리오가 문서를 발행하면 여기에 자동 등록됩니다. 실서비스에서는 MES 작업지시·EAM 정비오더와 양방향 연동됩니다.",
  /* ── 설비·로트 코드 스캔 ──
  ...
  scanRegistry: [
    ...
    { code: "FUR-03", type: "equip", label: "침탄로 3호기 — 온도 편차 진단",
      query: "침탄로 3호기 존별 온도 편차와 경도 영향을 진단해줘" },
    { code: "M-318", type: "order", label: "브래킷 M-318 — 설계 아웃라인 생성",
      agentId: "orchestration:2" },
    { code: "HB-0312", type: "lot", label: "브래킷 로트 HB-0312 — 품질 이력 조회",
      agentId: "agent-dbquery" },
    { code: "SUS304-2211", type: "material", label: "SUS304 코일 — 수입검사 성적서 판정",
      agentId: "orchestration:1" },
  ],
  /* ── 교대 인수인계 (3교대) ──
     현장에서 사고·품질 이슈가 가장 많이 새는 지점. AI가 그 조의 알람·조치·미결을
     모아 초안을 만들고, 담당자가 확인·보완해 다음 조로 넘긴다.
  ...
  shiftHandover: {
    ...
    shifts: [
      ...
      { id: "C", label: "야간조", time: "22:00–06:00" },
    ],
    currentId: "B",
    previous: {
      shiftId: "A", author: "김도현 반장", time: "14:02",
      items: [
        { type: "alarm",   text: "PRS-C03 진동 RMS 4.2mm/s 지속 — 3/22 계획정지에 베어링 교체 편성됨(HBP-보전-2026-102)" },
        { type: "action",  text: "침탄로 3호기 후단존 열전대 재교정 완료 — 편차 -7.8℃ → -6.9℃로 일부 개선" },
        { type: "pending", text: "2공장 프레스 라인 윤활 자동 도포 장치 시범 적용 3일차 — 표면 결함률 관찰 계속 필요" },
        { type: "pending", text: "협력사 검사성적서 9건 중 3건 판정 보류 — 두께 측정값 재확인 요청 발송함" },
        { type: "quality", text: "브래킷 로트 HB-0312 경도 58.4 HRC (하한 58.0 근접) — 전수 검사로 전환 유지" },
      ],
    },
    draftSeed: {
      alarms: [
        "침탄로 3호기 후단존 온도 편차 -7.1℃ — 관리 한계(-5℃) 초과 상태 지속",
      ],
      actions: [
        "PRS-C03 베어링 교체용 예비품 출고 확인 (자재팀 협조 완료)",
        "2공장 프레스 3호기 금형 교체(SMED) 27분 — 표준 25분 대비 2분 초과",
      ],
      pending: [
        "침탄로 3호기 최적 공정조건(후단존 +8℃·유지시간 +6분) 3로트 시험 열처리 — 야간조에서 착수 예정",
        "미매칭 설비 태그 1,834개 중 자동 확정 대상 1,312개 반영 승인 대기",
      ],
      quality: [
        "금일 공정 불량률 0.39% (주간 누계) — 전주 대비 0.03%p 개선",
      ],
    },
    note: "확정하면 다음 조의 '받은 인수인계'로 넘어갑니다. 실서비스에서는 MES 교대 마감과 연동됩니다.",
  },
  /* ── 결재 상신 전 AI 사전 검수 항목 (docType별) ──
     코어 기본값은 부동산 통계 항목이라 제조 문서에 맞게 재구성한다.
     status: 'pass'(자동 체크) | 'warn'(사용자가 근거 확인 후 체크해야 상신 가능) | 'fail'
     항목 수는 자유(코어가 지연 배열을 순환 적용). 세계관: HBP 문서번호·프레스·침탄로 */
  selfChecks: {
    report: [
      { id: 'docnum', label: '문서번호 형식', desc: 'HBP-부서-연도-일련번호 체계 검증', status: 'pass',
        detail: 'HBP-생산기술팀-2026-027 — 형식 정상',
        basis: '문서 헤더의 문서번호를 사내 문서관리규정 제12조 부여 체계(사업장코드-부서코드-연도-일련번호)와 대조한 결과 4개 구성요소가 모두 형식에 부합합니다. 연도(2026)·일련번호(027) 중복 없음 확인.' },
      { id: 'period', label: '집계 기간 명시', desc: '보고 대상 기간과 MES 집계 기준 시점 기재 여부', status: 'pass',
        detail: '2026.03.09 ~ 03.13 · MES 마감 기준 03.13 24:00',
        basis: '보고 기간과 MES 실적 마감 시점이 함께 표기되어 있어 재집계 시 동일한 수치가 재현됩니다. 마감 이후 입력분은 차주 보고에 반영된다는 단서도 포함되어 있습니다.' },
      { id: 'output', label: '생산 실적 수치 정합', desc: '생산량·가동률·불량률이 MES 집계와 일치하는지', status: 'pass',
        detail: '생산 12,360EA · 가동률 86.6% · 불량률 0.42% 일치',
        basis: 'MES 생산실적 API 조회 결과와 보고서 기재 수치를 대조해 생산량·가동률·불량률 3개 항목이 모두 일치함을 확인했습니다. 계획 대비 달성률(103%)도 계획값 12,000EA 기준으로 재계산해 검증했습니다.' },
      { id: 'defect', label: '불량 원인 기재', desc: '불량률 변동에 대한 원인·조치 서술 포함 여부', status: 'pass',
        detail: '치수 불량 개선 원인(금형 교체 M-204) 기재됨',
        basis: '전주 대비 불량률 변동에 대해 원인(2/18 금형 교체)과 후속 조치가 함께 서술되어 있습니다. 수치만 나열하고 해석이 빠진 보고서는 반려 사유에 해당하므로 이 항목을 점검합니다.' },
      { id: 'sign', label: '결재선 완성도', desc: '작성자·검토자·승인자 3단 서명 체계 확인', status: 'pass',
        detail: '3단 순차 서명 구조 확인됨',
        basis: '작성자(생산기술팀)→검토자(생산관리부장)→승인자(공장장) 순의 3단 결재선이 빠짐없이 구성되어 있습니다.' },
      { id: 'source', label: '데이터 출처 명시', desc: 'MES·SCADA 등 집계 원천 시스템 표기 여부', status: 'warn',
        detail: '출처(MES 생산실적 DB) 미기재 — 문서 하단 표기 권고',
        basis: '수치의 출처 시스템이 표기되지 않았습니다. 감사·이력 추적 시 어느 시스템 어느 시점 집계인지 확인할 수 없으므로 문서 하단에 출처와 조회 일시를 남기실 것을 권고합니다.' },
      { id: 'lotkey', label: '로트 키 연계 범위', desc: '공정조건 연계가 안 되는 구간의 명시 여부', status: 'warn',
        detail: '로트 키 미발행 29% 구간 주석 없음 — 각주 기재 권고',
        basis: 'MES 실적과 SCADA 공정조건을 잇는 로트 키가 발행되지 않은 구간이 29% 있습니다. 이 구간은 공정조건 분석에서 제외되므로, 수치를 그대로 인용할 때 오해가 없도록 각주로 범위를 밝히는 것을 권고합니다.' },
    ],
    meeting: [
      { id: 'docnum', label: '문서번호 형식', desc: 'HBP-부서-연도-일련번호 체계 검증', status: 'pass',
        detail: 'HBP-생산기술팀-2026-041 — 형식 정상',
        basis: '회의록 문서번호가 사내 부여 체계에 부합하며 동일 연도 내 일련번호 중복이 없습니다.' },
      { id: 'attend', label: '참석자 정보 완전성', desc: '부서·직위 포함 참석자 명단 기재 여부', status: 'pass',
        detail: '참석 6인 전원 부서·직위 표기됨',
        basis: '참석자 전원의 소속 부서와 직위가 표기되어 있어 후속 액션 아이템의 책임 소재가 명확합니다.' },
      { id: 'agenda', label: '안건·결정사항 구분', desc: '논의 내용과 결정사항이 분리 기재되었는지', status: 'pass',
        detail: '안건 3건 · 결정사항 4건 분리 기재',
        basis: '논의 경과와 확정된 결정사항이 별도 절로 구분되어 있습니다. 섞여 있으면 무엇이 확정인지 다툼이 생기므로 이 항목을 점검합니다.' },
      { id: 'action', label: '액션 아이템 담당·기한', desc: '조치 항목마다 담당자와 완료 기한 지정 여부', status: 'pass',
        detail: '액션 4건 전부 담당자·기한 지정됨',
        basis: '추출된 액션 아이템 4건 모두 담당자와 기한이 지정되어 있어 차기 회의에서 이행 점검이 가능합니다.' },
      { id: 'spec', label: '공정 수치 표기 정확성', desc: '언급된 공정조건·불량률 수치의 단위·기준 표기', status: 'warn',
        detail: '표면 결함률 0.14→0.19% 기준 시점 미표기 — 보완 권고',
        basis: '녹취에서 추출한 수치에 비교 기준 시점이 붙어 있지 않습니다. 전주 대비인지 전월 대비인지 불명확하면 후속 판단이 어긋날 수 있어 기준을 명시하시길 권고합니다.' },
      { id: 'confidential', label: '기밀 정보 포함 여부', desc: '협력사 단가·공정조건 레시피 등 대외비 항목 점검', status: 'pass',
        detail: '대외비 수치 미포함 — 사내 배포 가능',
        basis: '협력사 단가나 열처리 레시피 등 사외 유출 시 문제가 되는 항목이 본문에 포함되어 있지 않아 사내 배포 등급으로 처리 가능합니다.' },
    ],
    safety: [
      { id: 'docnum', label: '문서번호 형식', desc: 'HBP-부서-연도-일련번호 체계 검증', status: 'pass',
        detail: 'HBP-생산기술팀-2026-032 — 형식 정상',
        basis: '안전관리계획서 문서번호가 사내 부여 체계에 부합합니다.' },
      { id: 'law', label: '법령 근거 기재', desc: '산업안전보건법 등 근거 조항 인용 여부', status: 'pass',
        detail: '산업안전보건법 제36조(위험성평가) 인용 확인',
        basis: '위험성평가 의무 근거인 산업안전보건법 제36조가 인용되어 있습니다. 근거 없는 계획서는 감독 대응 시 효력을 인정받기 어렵습니다.' },
      { id: 'risk', label: '위험요인 평가 완전성', desc: '선택 위험요인별 빈도·강도·대책 기재 여부', status: 'pass',
        detail: '위험요인 3건 전부 강도·빈도·대책 기재',
        basis: '선택된 위험요인마다 심각도·발생빈도 등급과 구체적 저감 대책이 짝지어져 있어 평가표 요건을 충족합니다.' },
      { id: 'loto', label: '핵심 안전조치 누락 점검', desc: '설비 정비 작업의 LOTO(잠금·표지) 절차 포함 여부', status: 'pass',
        detail: 'LOTO 절차 및 2인 1조 원칙 명시됨',
        basis: '프레스·금형 관련 작업에서 사고 비중이 가장 높은 항목이 전원 차단 미조치입니다. LOTO(Lock Out/Tag Out) 절차와 2인 1조 원칙이 계획서에 포함되어 있음을 확인했습니다.' },
      { id: 'emergency', label: '비상 연락 체계', desc: '사고 발생 시 보고 경로와 연락처 등록 여부', status: 'pass',
        detail: '4단계 비상 연락 체계 확인됨',
        basis: '위험 상황 발생부터 관리자 보고까지 4단계 연락 체계가 순서대로 기재되어 있습니다.' },
      { id: 'training', label: '작업 전 교육 계획', desc: '해당 작업 투입 전 안전교육 실시 계획 기재', status: 'warn',
        detail: '작업 전 특별안전교육 일정 미기재 — 보완 권고',
        basis: '계획서에 교육 실시 여부만 있고 일정과 대상자가 특정되어 있지 않습니다. 실제 이행 점검이 어려우므로 일시·대상·교육시간을 기재하실 것을 권고합니다.' },
    ],
  },
  secureSuggestions: [
            ...
            sub: [
              {
                label: "가. 생산 실적",
                bullets: [
                  "CNC 3라인 브래킷 가공 <b>12,360EA</b> 생산 (계획 대비 103%)",
  ...
  adminContent: {
    ...
    MOCK_AIACT_LABELING: [
      {id:'lb-01',target:'AgentQ 채팅 응답',              type:'텍스트',method:'응답 하단 고지문 자동 삽입',                          enabled:true, coverage:100,  weekly:9840},
    ...
    MOCK_APIS: [
      {id:'api-001',name:'AgentQ Chat API',            endpoint:'/api/v1/chat',            version:'v1.2',auth:'Bearer Token',status:'활성',callsToday:18420,approvedDate:'2026-01-05'},
    ...
    MOCK_USAGE_HISTORY: [
      ...
      {id:'uh-006',user:'박태윤',dept:'생산기술팀',mode:'REPORT',query:'이번 주 CNC 라인 브래킷 12,360EA 생산, 수입검사 9로트 처리 완료를 주간 생산 보고서로 작성해줘',answer:'생산기술팀 주간 생산 실적 보고 | 보고 기간: 2026.02.17~02.21...',time:'2026-02-24 14:15',tokens:680,rating:5,errReport:false},
    ...
    MOCK_ANNOUNCEMENTS: [
      {id:1,title:'AgentQ AI 플랫폼 정식 오픈 안내',category:'공지',status:'Running',startDate:'2026-02-01',endDate:'2026-03-01',author:'임수진',views:452},
    ...
    MOCK_AIACT_SYSTEMS: [
      ...
      {id:'hi-005',name:'AgentQ 업무지원 챗봇',dept:'스마트팩토리 혁신 TF',status:'비해당',confirmedAt:'2026-01-28',manager:'서동현 팀장',
    ...
    AIACT_STD_PHRASE: '본 내용은 한빛정밀 생성형 AI 플랫폼(AgentQ)을 활용하여 작성되었습니다. 중요한 의사결정에는 담당자의 검토·확인이 필요합니다.',
  },
};
